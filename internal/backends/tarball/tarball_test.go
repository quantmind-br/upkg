...
func TestChooseBestExecutablePrefersCoreBinary(t *testing.T) {
	...
	best := backend.chooseBestExecutable(executables, "mod-desktop-0.0.12-linux-x86-64", installDir)
...
func TestScoreExecutablePenalizesSharedLibraries(t *testing.T) {
	...
	libScore := backend.scoreExecutable(libJack, "mod-desktop-0.0.12-linux-x86-64", installDir)
	binScore := backend.scoreExecutable(mainBinary, "mod-desktop-0.0.12-linux-x86-64", installDir)
